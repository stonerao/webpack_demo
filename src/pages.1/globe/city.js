...
class City extends Base {
    ...
    methods() {
        ...
        return {
            ...
            drawChinaMap() {
                //绘制中国地图 
                var mapLine = new MeshLineMaterial({
                    color: new THREE.Color("#ff000c"),
                    opacity: 1,
                    resolution: state.resolution,
                    sizeAttenuation: 1,
                    lineWidth: 0.2,
                ...
                for (let x = 0; x < features.length; x++) {
                    ...
                    console.log(features[x].properties.centroid)
                    for (let y = 0; y < coordinates.length; y++) {
                        ...
                        for (let z = 0; z < coordinates[y].length; z++) {
                            ...
                            for (let e = 0; e < p.length; e++) {
                                geometry.vertices.push(new THREE.Vector3(...p[e])); 
                            }
                        } 
                        var trail_line = new MeshLine()
                        trail_line.setGeometry(geometry);
                        var trail_mesh = new THREE.Mesh(trail_line.geometry, mapLine);
                        trail_mesh.frustumCulled = false;
                        trail_mesh.paramsType = "line"
                        state.scene.add(trail_mesh);
                    } 
